## function that targets by different threshold depending on whether is male/female
...
optimization_search = function(tau_vector,
                               demographic_data,
                               min_tau, 
                               max_target_rate, # [0,1] % of targets with tau>min_tau
                               rescale_demographic=TRUE,
                               seed=1){
  stopifnot(length(tau_vector)==dim(demographic_data)[1])
  stopifnot(data.table::between(max_target_rate, 0, 1))
  ## helper functions
  ...
  constrain_fun = function(x){
    return(sum(as.numeric(x>=0.5) ))
  }
  
  main_roi = function(){
    num_vars = length(tau_target)
    Objective = ROI::F_objective(F=opt_fun, n = num_vars)
    Constrain = ROI::F_constraint(constrain_fun, dir="<=", rhs=max_num_selected)
    opt = ROI::OP(objective = Objective,
                  maximum=TRUE, constraints = Constrain,
    ...
    solution = ROI_solve(opt,
                         solver="deoptimr", #"optimx" (not available for constrain),#"alabama" (very slow), #nlminb
                         max_iter= 100,
                         start =init_vars,
                         # method = "BFGS",
                         verbose=FALSE)
  ...
  max_num_selected = as.integer( max_target_rate*length(tau_target))
  print(sprintf("Selected %s of users with tau >= %s", max_num_selected, min_tau))
