...
# this following coyp from codes above and recycle the same parameters  
  print(penalty_value) 
  fit_fair <- balanced_causal_forest(X_train, Y_train, W_train,
                                     target.weights = as.matrix(Z_train),
  ...
  out = data.table(row_id = 1: dim(X_train)[1], 
...
# organize resutls into one row per penalty 
W.hat_train = mean(W_train) 

collect_balance_grf_results = function(target_vector){
...
for(target_rate in c(0.2,0.4,0.5, 0.6, 0.8)){
  ...
  tau_results = tau_penalty[, collect_balance_grf_results(is_targeted),  by=penalty_rate]
  
  
  tau_results_m = melt(tau_results, id.vars = c("penalty_rate"),
                       measure.vars = c("ipw",
                                        "ate_targeted",
                                        "ate_non_targeted",
  ...
  tau_benchmark = melt(tau_benchmark, id.vars = 'benchmark')
  tau_benchmark[, value := as.numeric(value)]
...
p_min = ggline(contour_data[target_rate>0.6],
               x="imbalance_z1",
               y="ate_targeted", 
               color="target_rate",
               legend="none",
